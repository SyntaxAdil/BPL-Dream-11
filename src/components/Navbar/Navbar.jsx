  ...
  return (
    <>
      <nav className="p-2 md:p-4 flex items-center justify-between  px-6 ">
        ...
        <button
          className="btn btn-success btn-ghost block md:hidden"
          onClick={() => setOpen((p) => !p)}
        >
          <Menu />
        </button>
      ...
      ></section>
      <aside
        className={`max-w-md w-full bg-white h-full shadow p-6 fixed  z-10 top-0 right-0 block md:hidden transition-all duration-300 ease-in-out  ${open ? "translate-x-0" : "translate-x-full"}`}
      ...
      >
        ...
        <ul className="flex flex-col mt-10 items-center  gap-8 font-medium px-2">
          {links}
          <button className="px-4 py-2 rounded-md border border-base-300 flex items-center justify-center gap-2 ">
            0 Coin
            <img src={COIN_IMAGE} alt="Double Dollar" />
          </button>
        </ul>
